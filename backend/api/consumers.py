...
class InterviewConsumer(AsyncWebsocketConsumer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._voice_confidence_samples = []
    ...
    async def _cleanup_connection(self):
        ...
        # Persist any recorded audio and voice confidence metrics
        try:
            await self._finalize_recordings()
        except Exception as e:
            logger.error(f"[Consumer] Failed to persist recordings: {e}", exc_info=True)
    ...
    async def _handle_binary_audio(self, bytes_data: bytes):
        ...
        # Process audio with TF processor (noise suppression + normalization + VAD + confidence)
        try:
            pcm16_data, is_speech, confidence = self.audio_processor.process_audio(
                bytes_data, 
                input_format='float32'
            )

            # Accumulate raw audio frames for recording
            if pcm16_data:
                self._recorded_audio_frames.append(pcm16_data)

            # Track per-frame voice confidence
            self._voice_confidence_samples.append(confidence)
            
            # Debug logging every 100 frames
            if self._audio_frame_count % 100 == 0:
                energy = self.audio_processor.get_energy(bytes_data)
                logger.debug(
                    f"[VAD] Frame {self._audio_frame_count}: "
                    f"energy={energy:.4f}, noise_floor={self.audio_processor._noise_floor_energy:.4f}, "
                    f"is_speech={is_speech}, confidence={confidence:.3f}, "
                    f"user_speaking={self._user_is_speaking}"
                )

            # Send periodic confidence updates to client (e.g., every 20 frames)
            if self._audio_frame_count % 20 == 0:
                try:
                    await self.send(text_data=json.dumps({
                    ...
                    # Don't fail the pipeline if UI message fails
                    logger.debug("[VAD] Failed to send voice confidence update to client", exc_info=True)
    ...
    def _finalize_recordings(self):
        """
        Persist recorded audio frames to disk and update interview analytics.
        Stores audio as a WAV file under MEDIA_ROOT/interviews/audio/.
        """
        if not self.interview or not self._recorded_audio_frames:
            return
        ...
        audio_dir = media_root / "interviews" / "audio"
        audio_dir.mkdir(parents=True, exist_ok=True)

        filename = f"{self.interview_id}.wav"
        full_path = audio_dir / filename

        # Write WAV file (mono, 16-bit PCM)
        with wave.open(str(full_path), "wb") as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)  # 16-bit
            wf.setframerate(self.audio_processor.target_sample_rate)
            for frame in self._recorded_audio_frames:
                wf.writeframes(frame)

        # Compute average voice confidence over the session
        avg_confidence = 0.0
        if self._voice_confidence_samples:
            avg_confidence = float(
                sum(self._voice_confidence_samples) / len(self._voice_confidence_samples)
            )

        # Update interview model with file path and confidence metric
        from core.models import Interview

        interview = Interview.objects.get(id=self.interview_id)
        relative_path = os.path.join("interviews", "audio", filename).replace("\\", "/")
        # Only set fields if they exist on the model (for safety with older migrations)
        update_fields = ["updated_at"]
        if hasattr(interview, "audio_recording"):
            interview.audio_recording.name = relative_path
            update_fields.append("audio_recording")
        if hasattr(interview, "average_voice_confidence"):
            interview.average_voice_confidence = avg_confidence
            update_fields.append("average_voice_confidence")
