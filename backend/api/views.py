...
from interview.analysis import run_interview_text_analysis
...
class InterviewEndView(APIView):
    ...
    def post(self, request, interview_id):
        ...
        try:
            ...
            # Run post-interview analysis (best-effort; errors are logged but do not fail the request)
            try:
                run_interview_text_analysis(interview)
            except Exception as analysis_err:
                logger.error(f"Post-interview analysis failed for {interview_id}: {analysis_err}", exc_info=True)
...
class InterviewMediaUploadView(APIView):
    """Upload recorded interview media (audio/video) for an interview."""

    def post(self, request, interview_id):
        """Attach uploaded media files to an interview."""
        try:
            interview = Interview.objects.get(id=interview_id)
        except Interview.DoesNotExist:
            logger.warning(f"Interview not found for media upload: {interview_id}")
            return Response(
                {'error': ERROR_MESSAGES['interview_not_found']},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        if video_file and hasattr(interview, 'video_recording'):
            interview.video_recording.save(video_file.name, video_file, save=False)
            updated_fields.append('video_recording')

        if audio_file and hasattr(interview, 'audio_recording'):
            interview.audio_recording.save(audio_file.name, audio_file, save=False)
            updated_fields.append('audio_recording')

        if updated_fields:
            interview.save(update_fields=updated_fields + ['updated_at'])

        logger.info(
            f"Media uploaded for interview {interview_id}: "
            f"{', '.join(updated_fields) if updated_fields else 'none'}"
        )
