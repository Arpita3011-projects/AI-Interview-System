...
import { createInterviewWebSocket, endInterview, reportCheating, uploadInterviewMedia } from '../services/api';
...
export default function InterviewPage({ resumeData, onComplete }) {
  ...
  const initializeMedia = async () => {
    try {
      ...
      // Setup MediaRecorder to capture combined audio + video for archival
      try {
        ...
        mediaRecorder.onstop = () => {
          ...
          mediaRecorderRef.current.recordedBlob = recordedBlob;
        };
        ...
        console.warn('MediaRecorder not available, video will not be archived:', err);
      }
  ...
  const connectWebSocket = () => {
    wsRef.current = createInterviewWebSocket(interview.id, {
      ...
      onVoiceConfidence: (value) => {
        setVoiceConfidence(value);
      },
    });
  };
  ...
  const handleInterviewEnd = async (wasTerminated = false) => {
    // Stop capture but keep blobs available
    cleanup();

    try {
      // Upload recorded media (if available)
      if (mediaRecorderRef.current?.recordedBlob) {
        try {
          await uploadInterviewMedia(interview.id, {
            videoBlob: mediaRecorderRef.current.recordedBlob,
          });
        } catch (uploadErr) {
          console.error('Failed to upload interview media:', uploadErr);
        }
      }
  ...
  const cleanup = () => {
    ...
    if (mediaRecorderRef.current?.mediaRecorder && mediaRecorderRef.current.mediaRecorder.state !== 'inactive') {
      try {
  ...
  return (
    <div>
      ...
      {voiceConfidence !== null && !terminated && (
        <div className="card" style={{ marginTop: '1rem' }}>
          <h3>Voice Confidence</h3>
          <p style={{ marginBottom: '0.5rem' }}>
            Estimated speaking confidence based on your voice characteristics.
          </p>
          <div style={{ height: '10px', background: '#eee', borderRadius: '4px', overflow: 'hidden' }}>
            <div
              style={{
                width: `${Math.round(voiceConfidence * 100)}%`,
                height: '100%',
                background: voiceConfidence > 0.7 ? '#16a34a' : voiceConfidence > 0.4 ? '#facc15' : '#dc2626',
