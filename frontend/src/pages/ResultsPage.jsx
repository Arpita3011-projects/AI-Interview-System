              ...
              <div style={{
                ...
                {data.evaluation.communication && (
                  <div style={{ marginTop: '1.5rem' }}>
                    ...
                    <p><strong>Score:</strong> {data.evaluation.communication.score_0_100}/100</p>
                    {data.evaluation.communication.justification && (
                      <p style={{ color: 'rgba(255,255,255,0.8)' }}>
                        {data.evaluation.communication.justification}
                      </p>
                    )}

                    {data.evaluation.communication.filler_words && (
                      <p style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)' }}>
                        <strong>Filler words:</strong>{' '}
                        ~{data.evaluation.communication.filler_words.total_count_estimate} estimated,
                        density ~{data.evaluation.communication.filler_words.density_per_min_estimate?.toFixed?.(1) ?? data.evaluation.communication.filler_words.density_per_min_estimate} / min
                      </p>
                    )}

                    {data.evaluation.communication.grammar && (
                      <p style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)' }}>
                        <strong>Grammar score:</strong> {data.evaluation.communication.grammar.score_0_100}/100
                      </p>
                    )}
                  </div>
                )}
                ...
                {/* Emotion & Sentiment */}
                {(data.evaluation.emotion || data.evaluation.sentiment) && (
                  <div style={{ marginTop: '1.5rem' }}>
                    <h4 style={{ marginBottom: '0.5rem' }}>😊 Emotion & Sentiment</h4>
                    {data.evaluation.emotion && (
                      <p style={{ marginBottom: '0.25rem', color: 'rgba(255,255,255,0.8)' }}>
                        <strong>Dominant emotion:</strong> {data.evaluation.emotion.dominant}
                      </p>
                    )}
                    {data.evaluation.sentiment && (
                      <p style={{ marginBottom: '0.25rem', color: 'rgba(255,255,255,0.8)' }}>
                        <strong>Overall sentiment:</strong> {data.evaluation.sentiment.overall}
                      </p>
                    )}
                  </div>
                )}
                ...
                {/* Technical section */}
                {data.evaluation.technical && (
                  <div style={{ marginTop: '1.5rem' }}>
                    <h4 style={{ marginBottom: '0.5rem' }}>💻 Technical Performance</h4>
                    <p style={{ color: 'rgba(255,255,255,0.8)' }}>
                      <strong>Depth:</strong> {data.evaluation.technical.depth_score_0_100}/100<br />
                      <strong>Correctness:</strong> {data.evaluation.technical.correctness_score_0_100}/100<br />
                      <strong>Relevance:</strong> {data.evaluation.technical.relevance_score_0_100}/100
                    </p>
                    {data.evaluation.technical.summary && (
                      <p style={{ color: 'rgba(255,255,255,0.8)' }}>
                        {data.evaluation.technical.summary}
                      </p>
                    )}
  ...
  data: PropTypes.shape({
    ...
    evaluation: PropTypes.shape({
      ...
      communication: PropTypes.shape({
        score_0_100: PropTypes.number,
        justification: PropTypes.string,
        filler_words: PropTypes.shape({
          total_count_estimate: PropTypes.number,
          density_per_min_estimate: PropTypes.number,
      ...
      emotion: PropTypes.shape({
        ...
        confidence_0_1: PropTypes.number,
      ...
      technical: PropTypes.shape({
        depth_score_0_100: PropTypes.number,
        correctness_score_0_100: PropTypes.number,
        relevance_score_0_100: PropTypes.number,
        summary: PropTypes.string,
        strengths: PropTypes.arrayOf(PropTypes.string),
