...
/**
 * Upload recorded interview media (video/audio) for an interview.
 */
export async function uploadInterviewMedia(interviewId, { videoBlob, audioBlob } = {}) {
  const formData = new FormData();
  ...
  if (audioBlob) {
    formData.append('audio', audioBlob, `interview_${interviewId}.wav`);
  }
  ...
  const response = await fetch(`${API_BASE}/interview/${interviewId}/media/`, {
    method: 'POST',
    body: formData,
  });
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to upload interview media');
  }
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'voice_confidence':
            callbacks.onVoiceConfidence?.(data.value);
            break;
          case 'error':
